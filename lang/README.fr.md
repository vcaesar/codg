...
# YOLO (installateur PowerShell natif)
irm https://raw.githubusercontent.com/vcaesar/codg/main/demo/boot.ps1 | iex
```

Tous (macOS, Linux ou Windows via Git Bash / MSYS2 / Cygwin / WSL) :
...
- Fournisseurs multi-modèles (40+ fournisseurs API et Pro, Custom URL API) et modèles locaux via openai-compat ou claude-compat, prise en charge des modèles gratuits d'Openrouter, Ollama, Nvidia et d'autres, utilisez-les via « /connect », « /models » ou « codg auth »
- Règles asynchrones et de modèle pour les multi-agents
- Compression des entrées/sorties, du contexte et des prompts pour économiser les tokens ; cache et règles pour réduire les coûts
...
Chine : Z.ai, Zhipu, Zhipu Coding, Kimi, Kimi Coding, DeepSeek, MiniMax, MiniMax China, Qwen, MiMo, Qiniu Cloud, Ali Coding, Ali Coding CN, Tencent Coding.
...
Pour l'utilisation du TUI, consultez la [documentation des commandes TUI](./docs/cli.fr.md), et tapez `/help` dans le TUI pour afficher les raccourcis et autres aides.
...
codg used                          # Afficher les limites d'utilisation et les statistiques API pour tous les fournisseurs
codg update                   # Mettre à jour la version de codg
codg updatep                  # Mettre à jour les définitions de fournisseurs
```
...
[options]
lite_mode = 2          # 0 = tous les agents, 2 = ensemble léger par défaut, 4 = agent unique
