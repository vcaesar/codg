...
# YOLO (네이티브 PowerShell 설치 프로그램)
irm https://raw.githubusercontent.com/vcaesar/codg/main/demo/boot.ps1 | iex
```

모두 (macOS, Linux 또는 Git Bash / MSYS2 / Cygwin / WSL 환경의 Windows):
...
프로젝트 디렉터리로 이동해 `codg`를 실행하고, "/init"로 프로젝트를 초기화하세요.
...
- 다중 에이전트를 위한 비동기 및 모델 규칙
- 입출력, 컨텍스트와 프롬프트를 압축해 토큰 절약, 캐시와 규칙으로 비용 절감
...
TUI 사용법은 [TUI 명령어 문서](./docs/cli.ko.md)를 참조하고 TUI에서 `/help`를 입력하여 키 바인딩 및 기타 도움말을 확인하세요.
